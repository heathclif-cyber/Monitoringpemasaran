...
## BUG-016: Superman — captcha loading / 500 & jaringan Railway ke portal

**Tanggal:** 2026-07-28  
**Invoice contoh:** `26.043/GKP-N1/BO/NJM/VII/2026` (data bisnis OK: 1 termin 750 jt, dokumen siap; `invoice.superman` kosong)

### Gejala
1. Dialog captcha **loading terus**, UI: `Request failed: 500`
2. Deklarasi Railway macet/gagal di tahap simpan (`NS_BINDING_ABORTED` di `/spp/store`) atau To Do (`getTodo` timeout) lalu session hangus
3. Setelah session invalid, captcha tidak muncul lagi

### Root cause (dua lapis)
1. **Kode captcha lama:** Playwright `page.goto(..., wait_until="networkidle")` ke `superman.ptpn1.co.id` sering **hang/timeout 60s** → exception tidak di-map → **HTTP 500**. Worker Playwright `max_workers=1` juga membuat captcha ngantri di belakang job deklarasi.
2. **Infrastruktur:** Dari **container Railway production**, connect ke portal Superman **intermittent** (kadang form isi sampai 89%, kadang captcha HTTP timeout 3× connect). Dari **PC lokal**, GET login page + captcha image biasanya **<1 detik**. Bukan salah isian invoice/dokumen.

### Mitigasi kode (deploy 2026-07-28)
| Commit | Isi |
|--------|-----|
| `845664c` | Tombol hapus termin di Input Pembayaran (koreksi input dobel) |
| `01162fd` | Fallback simpan `/spp/store` via **httpx** HTTP/1.1 |
| `b76a91e` / `2ec01df` | Timeout UI/job max **3 menit** (bukan 10) |
| `40f3448` | `getTodo` soft-fail — jangan robohkan job di 95% |
| `c337aaf` | Captcha: stop hang `networkidle`, worker 2, error → 502 |
| `3d6e128` / `da64d67` | Captcha **HTTP murni (httpx)**: start/refresh/verify + simpan cookie `storage_state`; HTTP/1.1 + retry 3× |
...
- `services/superman/filler.py` (store httpx fallback)
- `services/superman/progress.py` (wall-clock 180s server)
...
| `845664c` | Hapus termin di Input Pembayaran |
| `01162fd` | BUG-009/012: fallback store via httpx |
| `b76a91e` | Timeout deklarasi max 3 menit |
| `40f3448` | getTodo soft-fail |
| `3d6e128` / `da64d67` | BUG-016: captcha httpx + retry HTTP/1.1 |
