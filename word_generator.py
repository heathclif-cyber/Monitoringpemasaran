...
def generate_do_docx(do) -> io.BytesIO:
    ...
    ctr('DELIVERY ORDER (DO)', bold=True, ul=True, size=12)
    ctr(f"Nomor : {do.no_do}", sa=6)
    ...
    def add_line(lbl, val, bold=False):
        row = tbl.add_row()
        for i, c in enumerate(row.cells):
            _no_borders(c, False); _cw(c, CW[i]); _para_fmt(c.paragraphs[0])
        _run(_cp(row.cells[0]), lbl, True)
        _run(_cp(row.cells[1]), ':')
        _run(_cp(row.cells[2]), val, bold)

    add_line('No. Invoice', do.no_invoice)
    add_line('No. Kontrak', inv.no_kontrak)
    add_line('Tanggal DO', _date(do.tanggal_do))
    add_line('Kepada Unit', do.kepada_unit or '-')
    add_line('Barang', f"{k.komoditi} - {_id_fmt(k.volume, 2).rstrip('0').rstrip(',')} {k.satuan}")
    add_line('Status Bayar', f"Lunas (Tgl: {_date(do.tanggal_pembayaran)})" if do.tanggal_pembayaran else "Belum Lunas")
    add_line('Nominal Transfer', _rp(do.nominal_transfer))

    doc.add_paragraph()
    p_date = doc.add_paragraph(f"Makassar, {_date(do.tanggal_do)}")
    p_date.alignment = WD_ALIGN_PARAGRAPH.RIGHT
